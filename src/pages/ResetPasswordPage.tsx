...
export function ResetPasswordPage({ onNavigate }: ResetPasswordPageProps) {
  ...
  const passwordStrength = (password: string) => {
    ...
    if (password.length < 8) return { strength: 25, label: 'Weak', color: 'bg-[#fe0000]' };
    if (password.length < 12) return { strength: 50, label: 'Fair', color: 'bg-[#ffc000]' };
    if (password.length < 16) return { strength: 75, label: 'Good', color: 'bg-[#FB9228]' };
    return { strength: 100, label: 'Strong', color: 'bg-[#FF6B35]' };
  };
        ...
        <div className="hidden lg:flex flex-col justify-center items-center p-12 relative">
          <div className="max-w-md animate-fade-in">
            <div className="w-64 h-64 mb-8 bg-gradient-to-br from-[#FF6B35]/20 via-[#FB9228]/20 to-[#FFA500]/20 rounded-full flex items-center justify-center backdrop-blur-sm animate-pulse-slow">
              <Lock className="w-32 h-32 text-[#FB9228]" />
            </div>
              ...
              <div className="space-y-2 animate-fade-in" style={{ animationDelay: '0.1s' }}>
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full pl-12 pr-12 py-4 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-2xl focus:border-[#FB9228] focus:ring-4 focus:ring-[#FB9228]/20 dark:focus:ring-[#FB9228]/30 outline-none transition-all duration-300 text-gray-900 dark:text-white placeholder-gray-400"
                  ...
                  <button
                    ...
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#FB9228] transition-colors duration-300"
                  >
                ...
                {newPassword && (
                  <div className="space-y-2 mt-3">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className={`font-medium ${strength.strength >= 75 ? 'text-[#FF6B35]' : strength.strength >= 50 ? 'text-[#ffc000]' : 'text-[#fe0000]'}`}>
                        {strength.label}
                      </span>
                    </div>
